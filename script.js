...
async function searchOrInsertValue() {
    const value = document.getElementById('value').value;
    const response = await fetch('http://localhost:5000/search_or_insert', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function resetCache() {
    ...
    updateCacheContents();
    updateSearchHistory();
    clearStateDetails();  // Clear the state details section
}
...
async function fetchStateDetails(state_ut) {
    const response = await fetch(`http://localhost:5000/state_details/${state_ut}`);
